...
<div class="py-3">
      ...
      <div class="card border rounded-3 p-3 h-100">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
          <span class="rounded-2 p-2" style="background:{{ $s['bg'] }}">
            <i class="fas {{ $s['ic'] }}"
               style="color:{{ $s['tc'] }};font-size:14px;width:14px;text-align:center"></i>
          </span>

          <span class="badge rounded-pill"
                style="background:{{ $s['bg'] }};color:{{ $s['tc'] }};font-size:11px;">
            {{ $s['trend'] }}
          </span>
        </div>

        <div class="fs-3 fw-medium lh-1">
          {{ number_format($s['n']) }}
        </div>
  ...
  <div class="d-flex align-items-center gap-2 mb-3">
    <span class="text-uppercase text-muted fw-medium"
    ...
    <div class="col-md-4 col-6">
      <a href="{{ route($m['route']) }}"
        ...
        <span class="rounded-2 p-2 flex-shrink-0" style="background:{{ $m['bg'] }}">
          <i class="fas {{ $m['ic'] }}"
             style="color:{{ $m['ic_c'] }};font-size:13px;width:14px;text-align:center"></i>
        </span>
